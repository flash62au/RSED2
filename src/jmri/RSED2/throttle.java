...
import jmri.RSED2.logviewer.ui.LogViewerActivity;
...
public class throttle extends Activity implements android.gesture.GestureOverlayView.OnGestureListener {
    ...
    private boolean prefBt = false;
    private String prefBtDevice = "HC-05";
...
//        bRev.invalidate();
    ...
    // Handle messages from the Bluetooth serial thread TO this thread
    @SuppressLint("HandlerLeak")
    class bt_handler extends Handler {
        @Override
        public void handleMessage(Message msg) {
            String response_str = msg.obj.toString();
            char whichThrottle = response_str.charAt(0);
            ...
            if (!conAddr.equals("Not Set")) { // for commands that must have a loco selected
                switch (msg.what) {
                    case message_type.BT_THROTTLE: {
                        //Log.d("Engine_Driver", "R.S.E.D.: " + whichThrottle + " " + msg.arg1);
                        speedUpdateAndNotify(whichThrottle, msg.arg1);
                        //enable_disable_direction_and_loco_buttons(whichThrottle);
                        break;
                    }
                    case message_type.BT_FUNCTION: {
                        mainapp.sendMsg(mainapp.comm_msg_handler, message_type.FUNCTION, whichThrottle + "", msg.arg1, 1);
                        set_function_state(whichThrottle, msg.arg1);
                        vThrotScrWrap.playSoundEffect(SoundEffectConstants.CLICK);
                        break;
                    }
                    case message_type.BT_DIRECTION: {
                        if (msg.arg1==0) {
            ...
            switch (msg.what) { // for commands that don't need to have a loco selected
                case message_type.BT_ESTOP: {
                ...
                case message_type.BT_NEXT_THROTTLE: {
                                        ...
                                        if ((aChar>='0') || (aChar<='9')) { // got a valid digit
                                            ...
                                            if (lastKeyCount == (lastKeyCountNeedExtraKeys+1)) {  // got all three/two digits
                                                ...
                                                if (lastKeyChars[0]=='=') { // speed setting
                                                    ...
                                                    mainapp.sendMsg(mainapp.bt_msg_handler, message_type.BT_THROTTLE, whichThrottle + "", (MAX_SPEED_VAL_WIT * kbInt / 100));
                                                    ...
                                                    if (kbInt<=28) {  // otherwise just ignore it
                                                        ...
                                                        mainapp.sendMsg(mainapp.bt_msg_handler, message_type.BT_FUNCTION, whichThrottle + "", kbInt);
                                                    }
                                                }
                                                lastKeyChars[0] = ' ';
                                                lastKeyCount = 0;
                                            ...
                                            lastKeyCount = 0;
                                            lastKeyCountNeedExtraKeys = 0;
                                            j--; // allow it to be processed in the rest of the function
                                        }
                                    } else {
                                        // deal with the other keys... allow it to be processed
                                        lastKeyChars[0] = ' ';
                                        lastKeyCount = 0;
                                        lastKeyCountNeedExtraKeys = 0;

                                        if (aChar == 'A') { // Forward
                                            mainapp.sendMsg(mainapp.bt_msg_handler, message_type.BT_DIRECTION, whichThrottle + "", DIRECTION_FORWARD);
                                        } else if (aChar == 'S') { // Reverse
                                            mainapp.sendMsg(mainapp.bt_msg_handler, message_type.BT_DIRECTION, whichThrottle + "", DIRECTION_REVERSE);
                                        } else if (aChar == '0') { // stop
                                            mainapp.sendMsg(mainapp.bt_msg_handler, message_type.BT_STOP, whichThrottle + "", 0);
                                        } else if (aChar == '9') { // Estop
                                            mainapp.sendMsg(mainapp.bt_msg_handler, message_type.BT_ESTOP, whichThrottle + "", 0);
                                        } else if (aChar == '8') { // next Throttle
                                            mainapp.sendMsg(mainapp.bt_msg_handler, message_type.BT_NEXT_THROTTLE, whichThrottle + "", 0);
                                        }
                                    }
...
//                          addr = con.getTrailAddr();
    ...
    public boolean BTinit()
    {
        ...
        else
        {
            for (BluetoothDevice iterator : bondedDevices)
            {
                ...
                if(iterator.getName().equals(prefBtDevice))
                {
                    device=iterator;
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        prefs = getSharedPreferences("jmri.RSED2_preferences", 0);
...
//      displaySpeedSteps = false;
       ...
       webView = (WebView) findViewById(R.id.throttle_webview);
        ...
        //R.S.E.D. Start --------------------------------
        prefBtDevice = prefs.getString("prefBluetoothDevice", getApplicationContext().getResources().getString(R.string.prefRsedBluetoothDeviceDefaulValue));
        prefBt = !prefBtDevice.equals("");

        // put pointer to this activity's BT handler in main app's shared variable
        mainapp.bt_msg_handler = new bt_handler();
